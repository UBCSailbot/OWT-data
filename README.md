...
- `decoded_signals.csv` (Decoded form of `combined_can_frames.csv`) is **not** tracked in this repo. The files run to hundreds of MB each, which exceeds GitHub's 100 MB file size limit, so they are gitignored and regenerated locally from `combined_can_frames.csv` as needed.